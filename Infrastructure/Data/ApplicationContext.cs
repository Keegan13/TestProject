...
namespace Infrastructure.Data
{
    public class ApplicationContext : DbContext
    {
        ...
        protected void ConfigureProjects(EntityTypeBuilder<Project> proj)
        {
            ...
            proj.Property(x => x.Id)
                .HasColumnName("project_id");

            proj.Property(x => x.Name)
                .HasColumnName("project_name")
                .IsRequired()
                .HasMaxLength(150);

            proj.Property(x => x.Description)
                .HasColumnName("project_description")
                .HasMaxLength(450);

            proj.Property(x => x.StartDate)
                .HasColumnName("project_start_date");

            proj.Property(x => x.EndDate)
        ...
        protected void ConfigureDevelopers(EntityTypeBuilder<Developer> dev)
        {
            ...
            dev.Property(x => x.FullName)
                .HasColumnName("developer_fullname")
                .IsRequired()
                .HasMaxLength(150);

            dev.Property(x => x.Nickname)
        ...
        protected void ConfigureProjectAssignment(EntityTypeBuilder<ProjectAssignment> asgn)
        {
            ...
            asgn.Property(x => x.ProjectId)
                .HasColumnName("project_id");

            asgn.Property(x => x.DeveloperId)
                .HasColumnName("developer_id");

            asgn.HasOne(x => x.Project)
                .WithMany(x => x.ProjectAssignments)
                .HasForeignKey(x => x.ProjectId)
                .OnDelete(DeleteBehavior.Cascade);

            asgn.HasOne(x => x.Developer)
                .WithMany(x => x.ProjectAssignments)
        ...
        protected void ConfigureDeveloperTags(EntityTypeBuilder<DeveloperTag> devTag)
        {
            ...
            devTag.Property(x => x.TagId)
                .HasColumnName("tag_id");

            devTag.Property(x => x.DeveloperId)
                .HasColumnName("developer_id");

            devTag.HasOne(x => x.Developer)
                .WithMany(x => x.DeveloperTags)
                .HasForeignKey(x => x.DeveloperId)
                .OnDelete(DeleteBehavior.Cascade);

            devTag.HasOne(x => x.Tag)
                .WithMany(x => x.DeveloperTags)
